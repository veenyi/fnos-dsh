window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function PluginInventorySettingsTab({ list, setEnabled, t }) {
			...
			const toggle = (entry) => {
				const target = entry.enabled ? false : true;
				setToggling(entry.entryId);
				Promise.resolve().then(() => setEnabled(entry.entryId, target)).then(() => {
					setRequest((value) => value + 1);
				}, (error) => {
					console.error("pluginInventory.setEnabled failed", error);
				}).finally(() => {
					setToggling(null);
				});
			};
									...
									return (0, react_jsx_runtime.jsxs)("li", {
										...
										children: [(0, react_jsx_runtime.jsxs)("button", {
											...
											children: [(0, react_jsx_runtime.jsx)("code", {
												...
												type: "button",
												className: PluginInventorySettingsTab_module_css_default.toggleBtn,
												onClick: () => toggle(entry),
												disabled: toggling === entry.entryId,
												"data-enabled": entry.enabled ? "true" : "false",
												children: toggling === entry.entryId ? t("toggling") : entry.enabled ? t("disableAction") : t("enableAction")
											})]
										}) : null]
									}, entry.entryId);
								})
