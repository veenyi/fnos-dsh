...
*/
function dialogNodeBin() {
	// dsh-desktop patch: koffi 的 FFI 调用（view/call）在 Electron 内置 V8 下触发 napi fatal error，
	// worker 必须用独立官方 Node（DSH_DIALOG_NODE_BIN）运行；NAS 版无此变量则回退 process.execPath。
	return process.env.DSH_DIALOG_NODE_BIN || process.execPath;
}
function spawnDialogWorker(data) {
	...
	if (!import.meta.url.endsWith(".ts")) return spawn(dialogNodeBin(), [fileURLToPath(new URL("./worker.cjs", import.meta.url))], {
		env,
		stdio,
		windowsHide: true
	});
	return spawn(dialogNodeBin(), [
		"--import",
